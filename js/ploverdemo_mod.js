...
/**
 * Đối tượng này sẽ lưu trữ ánh xạ giữa shorthand mã khóa và ký tự qwerty với dữ liệu được nhập từ tệp json bên ngoài.
 * Có một vấn đề trong đó chrome sẽ không tải các tệp json cục bộ, vì vậy các tệp phải được lưu trữ để dữ liệu được nhập vào chrome.
 * @see jQuery's <a href="http://api.jquery.com/jQuery.getJSON/">getJSON documentaion</a>.
 * @see The <a href="http://code.google.com/p/chromium/issues/detail?id=40787">chrome bug report</a>.
 */
...
// in the mod file, we don't need the function below because we don't have virtual keyboard
...
function resetKeys() {
    ...
    // we have no virtual keyboard so comment those below
...
function Word(strokesParam) {
    ...
    if (strokes.length > 0) {
        ...
        string = string.slice(0, -1); // remove last '/' from string
    }
     ...
     */
    this.removeStroke = function() { // Actually removed last stroke
        strokes.pop();
 ...
 */
 // comment function below because we don't need it
...
$(document).keydown(function(event) {
    if (checkIsActive()) {
        ...
        // we don't have virtual keyboard so disable those below
        // Update the display
        ...
        if (isSteno) {
            ...
            if ($.isEmptyObject(downKeys)) { // if no more keys are being pressed down, this is the end of the stroke.
                ...
                if (chord.toKeyCodes()[ctrlKey]) {
                    if (chord.toKeyCodes()[cKey] && chord.toKeyCodes()[ctrlKey]) { // if the combination is Ctrl+C mean "copy"
                        temp = window.getSelection().toString() // get the selection - the string you want to copy
                        document.execCommand('copy') // call clipboard api for copy action
                    }
                    if (chord.toKeyCodes()[sKey] && chord.toKeyCodes()[ctrlKey]) { // if the combination is Ctrl+S mean "cut" - dunno why don't use Ctrl+X 
                        let startPos = document.getElementById('output').selectionStart // get first index of selected string in textarea
                        temp = window.getSelection().toString() 
                        translatedString = $("#output").val()   // get string from output 
                        translatedString = translatedString.substring(0, startPos) + translatedString.substring(startPos + temp.length, translatedString.length) // prune the selected string from original string
                        translatedStringArrayUndo.push(translatedString) // save modified string for undo action 
                        $("#output").html(translatedStringArrayUndo[translatedStringArrayUndo.length - 1])  // set output string as the last string u push into undo-array
                        document.execCommand('copy') // copy selected string into clipboard
                    }
                    if (chord.toKeyCodes()[vKey] && chord.toKeyCodes()[ctrlKey]) {  // if the combination is Ctrl+V mean "paste"
                        let startPos = document.getElementById('output').selectionStart
                        translatedString = $("#output").val()
                        // temp below only have value when u use ctrl+C or Ctrl+S first or nothing happen
                    ...
                    if (chord.toKeyCodes()[uKey] && chord.toKeyCodes()[ctrlKey]) { // like Ctrl+S without copy selected text to clipboard
                        let startPos = document.getElementById('output').selectionStart
                    ...
                    if (chord.toKeyCodes()[qKey] && chord.toKeyCodes()[ctrlKey]) { // undo 
                        if (translatedStringArrayUndo.length > 0) {
                            ...
                            $("#output").html(translatedStringArrayUndo[translatedStringArrayUndo.length - 1]) 
                            setCaretToPos(document.getElementById("output"), $("#output").val().length) // change cursor position
                        }
                    }
                    if (chord.toKeyCodes()[eKey] && chord.toKeyCodes()[ctrlKey]) { // redo
                    ...
                    if (chord.toKeyCodes()[116] && chord.toKeyCodes()[ctrlKey]) { // a combination that reload website
                        location.reload(true)
                    }
                } else {
                    chords.push(chord); 

                    // note: this condition "dictionary[words[words.length - 1].toString() + '/' + chord.toRTFCRE()]" is to check if the word from words
                    // append with chord in RTFCRE format is exists in the dictionary
                    ...
                    } else if (words.length > 0 && chord.toRTFCRE() === '*') { // delete key "*" on steno keyboard if word exists (not empty)
                        words[words.length - 1].removeStroke(); // remove last stroke from last word
                        if (words[words.length - 1].toString() === '') { // after delete last stroke, if word becomes empty, remove word 
                            words.pop();
                        }
                    } else if (word.toEnglish() !== '') { // if it an English word, push to word without needing to add stroke
                        words.push(word);
                    }

                    let translatedString = '';
                    let startPos = document.getElementById('output').selectionStart
                    if (chord.toRTFCRE() === '*') { // delete word from output (display) from 
                        translatedString = $("#output").val()
                        ...
                        let index
                        for (let i = firstString.length; i >= 0; i--) { // why this shit count from array length index, array is zero based but length is one-based
                            if (firstString[i] !== ' ') { // this shit is bugs when out of array bounds (in some case)
                                index = i - 1
                                break
                            }
                        }
                ...
                // code below to generate sample paragraph to type follow
                ...
                checkCorrectArr = []
                // code generate paragraph end here
            }
...
function setCaretToPos(input, pos) {
    setSelectionRange(input, pos, pos); // input as element which call setSelectionRange(pos, pos)
}